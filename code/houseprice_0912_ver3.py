...
bts_train_x4, bts_train_y4 = bootstrap_data(train_x_scaled, train_y)
bts_train_x5, bts_train_y5 = bootstrap_data(train_x_scaled, train_y)

# ElasticNet 모델 및 그리드 서치
eln_model = ElasticNet()
param_grid_eln = {'alpha': np.arange(10.0, 100.0, 10), 'l1_ratio': [0.9, 0.925, 0.95, 0.975, 1.0]}
grid_search = GridSearchCV(estimator=eln_model, param_grid=param_grid_eln, scoring='neg_mean_squared_error', cv=5)

bts_eln_models = []
for bts_x, bts_y in zip([bts_train_x1, bts_train_x2, bts_train_x3, bts_train_x4, bts_train_x5],
                        [bts_train_y1, bts_train_y2, bts_train_y3, bts_train_y4, bts_train_y5]):
...
# 블렌더 모델 (RandomForestRegressor 사용)
blender_model = RandomForestRegressor(n_estimators=100)
...
sub_df.to_csv(os.path.join(output_dir, "sample_submission_ver3.csv"), index=False)
